...
@app.post('/delete')
async def delete_video(request: Request, file_name: str = Form(...)):
    try:
        response = supabase.storage.from_(SUPABASE_BUCKET).remove([file_name])
        if response.status_code >= 400:
            message = f"Error deleting video: {response.text}"
        else:
    ...
    return templates.TemplateResponse('upload.html', {'request': request, 'message': message, 'videos': videos})
